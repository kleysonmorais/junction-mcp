...
|---|---|
| Daily | `activity` |
| Session | `sleep`, `sleep_cycle`, `workouts`, `body`, `meal`, `menstrual_cycle`, `electrocardiogram` |
| Single | `profile` |

> **No `workout_stream` summary resource:** `/summary/workout_stream/{user_id}` does not exist (404s). Per-workout raw streams are fetched via a different, non-parametrized path: `GET /timeseries/workouts/{workout_id}/stream`.

```http
GET /v2/summary/activity/{user_id}?start_date=YYYY-MM-DD&end_date=YYYY-MM-DD
GET /v2/summary/sleep/{user_id}?start_date=...&end_date=...
GET /v2/summary/workouts/{user_id}?start_date=...&end_date=...
GET /v2/summary/body/{user_id}?start_date=...&end_date=...
...
GET /v2/summary/profile/{user_id}   # single, no date range needed
```
...
| Discrete | Body | `body_fat`, `body_weight` |
...
```http
GET /v2/timeseries/{user_id}/heartrate?start_date=...&end_date=...
...
GET /v2/timeseries/{user_id}/body_temperature?...
...
### Introspection
```http
GET /v2/introspect/historical_pull     # backfill job status, filterable by user_id/provider
```

> There is no separate "which resources are available for a user" introspection endpoint on this sandbox — use `GET /v2/user/providers/{user_id}` (see Key Concepts above) to see connected providers and their resource availability.