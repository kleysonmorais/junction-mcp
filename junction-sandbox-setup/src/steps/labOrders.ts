...
/**
 * Simulation progresses asynchronously server-side; the redraw child order only
 * exists once the initial order reaches redraw_available. Poll until it appears.
 */
async function waitForRedrawChild(
  client: JunctionClient,
  orderId: string,
  timeoutMs = 90_000,
): Promise<{ refreshed: Junction.ClientFacingOrder; redrawChild: Junction.ClientFacingOrderInTransaction | undefined }> {
  const deadline = Date.now() + timeoutMs;
  let refreshed = await client.labTests.getOrder({ orderId });
  let redrawChild = refreshed.orderTransaction?.orders?.find((o) => o.origin === "redraw");
  while (!redrawChild && Date.now() < deadline) {
    await sleep(3000);
    refreshed = await client.labTests.getOrder({ orderId });
    redrawChild = refreshed.orderTransaction?.orders?.find((o) => o.origin === "redraw");
  }
  return { refreshed, redrawChild };
}
    ...
    try {
      switch (def.kind) {
        ...
        case "redraw": {
          ...
          const { refreshed, redrawChild } = await waitForRedrawChild(client, order.id);
          orders[def.key] = {
            orderId: order.id,
