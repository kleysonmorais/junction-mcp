...
Local MCP client (Claude Desktop / Inspector)
  └─ stdio subprocess (bin/stdio.ts) ──► same shared tools ──► Junction
```

One transport-agnostic core, three entry points. `registerJunctionTools()` in
[lib/mcp/server.ts](../lib/mcp/server.ts) binds the 8 tools to a
`JunctionClient` instance; the HTTP routes, the stdio binary, and the agent all
reuse it unchanged. Only *how the Junction key arrives* differs per entry
point.
...
The LLM vendor is one line in [app/api/chat/route.ts](../app/api/chat/route.ts)
...
[bin/stdio.ts](../bin/stdio.ts) connects the same tool registry to the official
SDK's `StdioServerTransport`. Local MCP clients (Claude Desktop, MCP Inspector)
...
- **Origin validation** ([lib/security/origin.ts](../lib/security/origin.ts)) —
...
- **Key resolution** ([lib/auth/resolve-key.ts](../lib/auth/resolve-key.ts)) —
...
[junction-sandbox-setup/](../junction-sandbox-setup/) seeds the sandbox this
